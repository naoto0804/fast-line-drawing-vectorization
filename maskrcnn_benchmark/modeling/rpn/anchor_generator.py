    ...
    def grid_anchors(self, grid_sizes):
        ...
        for size, stride, base_anchors in zip(
            ...
            shifts_x = torch.arange(
                0, grid_width * stride, step=stride, dtype=torch.float32,
                device=device
            )
            shifts_y = torch.arange(
                0, grid_height * stride, step=stride, dtype=torch.float32,
                device=device
            )
        ...
        #     image = np.ones((128, 128, 3)) * 255
        #     colors = [tuple(int(y * 255) for y in cm.jet(x)[:-1]) for x in
        #               np.linspace(0.0, 1.0, len(anchor))]
        #
        #     for a, color in zip(anchor, colors):
        #         a = a.cpu().to(torch.int64)
        #         top_left, bottom_right = a[:2].tolist(), a[2:].tolist()
        #         image = cv2.rectangle(
        #             image, tuple(top_left), tuple(bottom_right), color, 1)
        #     cv2.imwrite("anchors_{:d}.png".format(i), image)
