# https://github.com/facebookresearch/maskrcnn-benchmark/pull/150/files
# https://github.com/facebookresearch/maskrcnn-benchmark/blob/7f22baab128958e07977fd6e1dcb92a097722477/maskrcnn_benchmark/structures/segmentation_mask.py
import numpy as np
...
class Mask(object):
    ...
    def __init__(self, segm, size, mode):
        ...
        if isinstance(segm, Mask):
            ...
            if type(segm) == list:
                # polygons
                mask = Polygons(segm, size, 'polygon').convert('mask').to(
                    dtype=torch.float32)
    ...
    def resize(self, size, *args, **kwargs):
        width, height = size
        scaled_mask = interpolate(self.mask[None, None, :, :], (height, width),
                                  mode='nearest')[0, 0]
...
class Polygons(object):
    ...
    def __init__(self, polygons, size, mode):
        ...
        if isinstance(polygons, list):
            polygons = [torch.as_tensor(p, dtype=torch.float32) for p in
                        polygons]
